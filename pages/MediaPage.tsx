...
import { Search, Filter, Image as ImageIcon, Grid, List, Download, Share2, MoreVertical, Plus, ShieldCheck, AlertTriangle, Sparkles, User, BrainCircuit, Loader2, CheckCircle2 } from 'lucide-react';
...
const MediaPage: React.FC = () => {
  ...
  // Dynamic Assets from Shoot System
  ...
  const handleBatchAudit = async () => {
    ...
    // Simulate campaign-wide reasoning
    await new Promise(resolve => setTimeout(resolve, 3000));
    setIsBatchScanning(false);
    setSelectedIds(new Set());
    alert("Campaign Batch Audit Complete. Compliance reports synchronized.");
  };
    ...
    <div className="p-8 md:p-12 max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
         <div>
            ...
            <p className="text-warmgray">Managing global assets for {brand.name}. {liveAssets.length > 0 && <span className="text-sage font-bold">• {liveAssets.length} New Captures</span>}</p>
         </div>
         <div className="flex items-center gap-4">
            {selectedIds.size > 0 && (
              <button 
                ...
                className="flex items-center gap-2 bg-sage text-white px-8 py-4 rounded-full text-[10px] uppercase font-bold tracking-widest hover:scale-105 transition-all shadow-lg animate-in slide-in-from-right-2"
              >
      ...
      <div className={`grid ${view === 'grid' ? 'grid-cols-1 md:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1'} gap-8`}>
         {filteredAssets.map((asset, i) => (
           <div 
             ...
             className={`group bg-white border rounded-[32px] overflow-hidden hover:shadow-2xl transition-all duration-500 relative cursor-pointer ${selectedIds.has(asset.id) ? 'border-sage ring-2 ring-sage/20' : 'border-[#E5E1D8]'} ${view === 'list' ? 'flex items-center p-4' : ''}`} 
           ...
           >
              ...
              <div className={`${view === 'grid' ? 'aspect-square' : 'w-24 h-24'} overflow-hidden relative flex-shrink-0 rounded-[20px] m-4 bg-ivory`}>
                 ...
                 {asset.cat === 'Live Captures' && (
                    <div className="absolute inset-0 bg-sage/10 animate-pulse pointer-events-none" />
                 )}
              </div>
              <div className={`p-6 ${view === 'list' ? 'flex-1 flex items-center justify-between' : 'space-y-4'}`}>
                 <div>
                    ...
                    <p className="text-[10px] uppercase font-bold text-sage tracking-widest mt-1 flex items-center gap-1">
                      {asset.cat === 'Live Captures' && <div className="h-1 w-1 bg-sage rounded-full animate-ping" />}
                      {asset.cat}
                    </p>
                 </div>

...
import { Send, Sparkles, Loader2, Mic, PhoneOff, FileText, X, BrainCircuit } from 'lucide-react';
...
const ChatPage: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: `Hello. I have analyzed the SS25 trajectory for ${brand.name}. How can I assist with your strategy today?` }
  ]);
                ...
                <div className="p-8 max-w-4xl mx-auto space-y-4">
                    ...
                    {isSummarizing ? (
                        <div className="flex items-center gap-3 py-4">
                            ...
                            <p className="text-sm italic text-warmgray font-serif">Synthesizing executive insights...</p>
                        </div>
                    ) : (
           ...
           {messages.map((msg, i) => (
             <div key={i} className={`flex gap-6 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`h-12 w-12 rounded-2xl flex items-center justify-center flex-shrink-0 border ${msg.role === 'assistant' ? 'bg-charcoal text-white' : 'bg-[#E5E1D8] overflow-hidden'}`}>
                  {msg.role === 'assistant' ? <Sparkles size={20} /> : <img src="https://i.pravatar.cc/100?u=12" className="w-full h-full object-cover" alt="User" />}
                </div>

  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="font-serif text-3xl font-medium">Events</h2>
        ...
        <div className="flex items-center gap-4">
           <div className="relative">
              ...
              <input type="text" placeholder="Search events..." className="pl-10 pr-4 py-2 bg-white border border-[#E5E1D8] rounded-full text-sm focus:outline-none focus:ring-1 focus:ring-[#1A1A1A] w-64" />
           </div>
           <button className="p-2 border border-[#E5E1D8] rounded-full hover:bg-white transition-colors"><Filter size={18} className="text-[#8C8C8C]" /></button>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E5E1D8] rounded-3xl overflow-hidden shadow-sm">
        <div className="border-b border-[#E5E1D8] px-8 flex">
           {tabs.map(tab => (
             <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-4 text-xs uppercase tracking-widest font-bold border-b-2 transition-all ${activeTab === tab ? 'border-[#1A1A1A] text-[#1A1A1A]' : 'border-transparent text-[#8C8C8C] hover:text-[#1A1A1A]'}`}>{tab}</button>
           ))}
        </div>
        <div className="p-0">
                ...
                <tr key={i} className="hover:bg-[#F9F7F2] transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-[#E5E1D8] rounded-lg overflow-hidden"><img src={eventImages[i]} className="w-full h-full object-cover" alt={ev.name} /></div>
                      <span className="text-sm font-semibold">{ev.name}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-sm text-[#8C8C8C]">{ev.loc}</td>
                  <td className="px-8 py-5"><span className="px-2 py-1 text-[10px] font-bold uppercase rounded bg-green-100 text-green-700">{ev.status}</span></td>
                  <td className="px-8 py-5 text-sm text-[#8C8C8C]">{ev.date}</td>
                  <td className="px-8 py-5 text-right"><button className="p-2 hover:bg-[#E5E1D8] rounded-full opacity-0 group-hover:opacity-100"><MoreHorizontal size={18} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
